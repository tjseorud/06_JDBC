...
public class JDBCExample2 {
	public static void main(String[] args) {
		 ...
		 * 장점 2 :? 값 대입시 자료형에 맞는 리터럴 표기법으로 자동 변환된다
		 *  ex)?에 int 대입 ->NUMBER 타입 리터럴(123)
		 *  ex)?에 String 대입 ->CHAR 타입 리터럴('123')(양쪽에 '' 자동추가)
		 * 장점 3 :Statement의 문제점인 SQL Injection을 방지할 수 있다
		 *  ->Statement는 문자열 결합방식으로 SQL을 만듦
		 *   ->결합되는 문자열이 SQL명령어 또는 '값' 모두가능
		 * 
		...
		try {	
			 ...
			 * ->매번 수동으로 끔
			 * 왜? 개발자가 트랜잭션을 마음대로제어하기 위해서*/
			conn.setAutoCommit(false);
			
			/* PreparedStatement 객체 생성
			 * ->객체생성과 동시에 SQL을 매개변수로 전달해서 담아둠 ->SQL 파악하기 위해서*/
			pstmt =conn.prepareStatement(sql);	/*?(placeholder)에 알맞은 값 세팅*/
			...
			/*result 값에 따라서 SQL 성공/실패 여부 출력 + 트랜잭션 제어(commit/rollback)*/
			if(result >0) {
